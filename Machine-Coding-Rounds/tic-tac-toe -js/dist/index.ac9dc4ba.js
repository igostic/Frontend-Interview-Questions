...
function startGame(type) {
    ...
    for(let i = 0; i < 9; i++){
        ...
        (box || boxes[i]).addEventListener("click", handleClick, {
            once: true
        });
    }
}
...
function handleClick(e) {
    ...
    if (win) gameOver("win", currentSelection);
...
function gameOver(type, currentSelection) {
    if (type === "win") gameResult.innerText = currentSelection + " wins the match";
    else gameResult.innerText = "Match Draw";
...
function isGameDraw() {
    ...
    return [
        ...boxes
    ].every((box)=>box.innerText === "X" || box.innerText === "O");
}
